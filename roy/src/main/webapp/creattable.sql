...
COMMENT ON COLUMN BCOMMENT.c_num IS '댓글번호';

COMMENT ON COLUMN BCOMMENT.c_cnt IS '댓글내용';

COMMENT ON COLUMN BCOMMENT.c_date IS '댓글 작성일';

COMMENT ON COLUMN BCOMMENT.b_num IS '게시글번호';

COMMENT ON COLUMN BCOMMENT.m_id IS '아이디';

COMMENT ON TABLE BCOMMENT IS '댓글';

CREATE UNIQUE INDEX 엔터티1_PK4 ON BCOMMENT
( c_num );
...
COMMENT ON COLUMN RFILE.f_num IS '파일번호';

COMMENT ON COLUMN RFILE.f_name IS '파일명';

COMMENT ON COLUMN RFILE.f_path IS '경로명';

COMMENT ON TABLE RFILE IS '이미지파일';

CREATE UNIQUE INDEX 엔터티1_PK5 ON RFILE
( f_num );
